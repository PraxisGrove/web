...
export const GridScan: React.FC<GridScanProps> = ({
  ...
  return (
    ...
    >
      {showPreview && (
        <div className="pointer-events-none absolute bottom-3 right-3 h-[132px] w-[220px] overflow-hidden rounded-lg border border-white/25 bg-black font-sans text-[12px] leading-[1.2] text-white shadow-[0_4px_16px_rgba(0,0,0,0.4)]">
          <video
            ref={videoRef}
            muted
            playsInline
          ...
          />
          <div className="absolute left-2 top-2 rounded-[6px] bg-black/50 px-[6px] py-[2px] backdrop-blur-[4px]">
            {enableWebcam
              ? modelsReady
                ? uiFaceActive
                  ? 'Face: tracking'
